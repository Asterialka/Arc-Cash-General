...
void replace(cache_ARC *arc, long long int page, int *p)
{
    assert(( p != NULL) && (*p < arc->size_ARC) && "incorrect parament p in replace");
    assert((p != NULL) && (arc->T1 != 0) && (arc->T2 != 0) && (arc->B1 != 0) && (arc->B2 != 0) && "Was given NULL pointer");
    if ((arc->T1->now_size >= 1) && ((((find_element(arc->B2, page) != NULL) && (arc->T1->now_size == *(p)) || (arc->T1->now_size > *(p))))))
    {
        popBack(arc->T1);
...
cache_ARC* ARC(cache_ARC *ARC, long long int page, int *p, int *hit)
{
    ...
    if (b1 != NULL)
    {
        *(p) = min(ARC->size_ARC, *(p) + max(ARC->B2->now_size / ARC->B1->now_size, 1));
        replace(ARC, page, p);
    ...
    if (b2 != NULL)
    {
        *(p) = max(0, *(p) - max(ARC->B1->now_size / ARC->B2->now_size, 1));
        replace(ARC, page, p);
